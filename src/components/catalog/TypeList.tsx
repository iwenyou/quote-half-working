...
import { saveProductType, deleteProductType } from '../../services/catalogService';
...
export function TypeList({ types }: TypeListProps) {
  ...
  const handleSave = async (type: ProductType) => {
    try {
      await saveProductType(type);
      showSuccess('Product type saved successfully');
